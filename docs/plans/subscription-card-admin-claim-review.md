...
- ✅ **§4a** New stricter guard `requireManageAdjustments` (`manage-subscription-adjustments-auth.ts`) — balance edits require `adminScope === 'SUPER'`; the `SUPER_ADMIN` *role* alone no longer suffices (it still gates plan config via `requireManageSubscriptions`). Unit-tested for the separation.
- ✅ **§4b** Manual adjustment now requires a free-text `note` (min 8 chars), persisted to the audit metadata.
- ✅ **§4c** Per-row inline adjust forms removed from the admin subscriptions table; replaced by a single de-emphasized **"Support override"** panel that only renders when the caller holds the new permission (`GET /api/admin/subscriptions` now returns `capabilities.canAdjustCredits`), with a required reason field.
