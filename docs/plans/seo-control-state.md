...
| SEO-004 | Booking-query canonical variants | Canonicalisation / crawl control | **DEPLOYED — VERIFIED IN PRODUCTION 2026-08-20** | 2026-08-17 | Clean `/book` stays indexable and in the sitemap. Any non-tracking booking query state now serves `noindex, follow` with the clean `/book` canonical; public doctor/service preselection CTAs navigate as buttons instead of exposing parameterized anchors. Tracking-only variants (UTM, `gclid`, and Google's own `srsltid` / `_gl` / `wbraid` / `gbraid`) retain the clean page's normal indexability. Crawl gate is `isBookingWorkflowHref` sitewide — any wizard parameter, not just the service+doctor pair. `tsc --noEmit` and `eslint` clean; 164/164 tests across `lib/seo`, `lib/routing`, `components/booking`. Local rendered HTML verified: clean `/book` and `/czechia/cs/book` = `index, follow`; `?doctor=` = `noindex, follow`; `?srsltid=`, `?_gl=`, `?utm_source=` = `index, follow`; every variant canonical to the clean `/book` with the full 6-locale hreflang cluster + `x-default`; `/gp-consultation-online`, `/doctors`, `/services/gp-consultation` and `/book` each served zero `/book?` anchors. | Production still serves the prior `index, follow` + clean-canonical policy until this batch is deployed | None. Live probes 2026-08-20 confirm `?doctor=`/`?service=`/`?lang=` = `noindex, follow` + clean canonical, `?utm_*` = `index, follow`, and zero `/book?` or `/doctors?` anchors in served HTML on `/doctors`, `/book`, a doctor profile and `/portugal/pt/doctors`. GSC's "Alternate page with proper canonical tag" validation FAILED 2026-08-18 — expected: the URLs still resolve 200 with a foreign canonical by design, so that validation can never pass. Historical rows decay on recrawl; do not re-run validation |
...
| SEO-012 | Two `<h1>` elements on all 33 country/locale `/doctors` URLs | On-page / duplication | **IMPLEMENTED LOCALLY — AWAITING DEPLOY** | 2026-08-19 | The directory hero lived inside `DoctorTeamTemplate`, which sits inside the page's `<Suspense>` boundary. Every `[country]/[lang]` route is `ƒ` (server-rendered on demand — `next build` emits zero prerendered HTML for them), so React streams the fallback AND the resolved child into the same response: production HTML carried the identical `<h1>` at byte 62,689 (fallback) and 199,968 (inside `<div hidden id="S:0">`). Hero extracted to a server-rendered `DoctorTeamHero` above the boundary — correct in both render modes, unlike emptying the fallback (which would resurrect the 912cd22d "no doctor links in HTML" bug if these routes ever go static). Hero's availability count is now the full roster rather than the filtered subset; it sits above the filter bar. Layout verified pixel-identical to production at 1280px and 375px (hero top 99/91, height 632/740, grid top 731) after restoring the `overflow-x: clip` context the hero lost when it left `.gh-medical-pattern` | Production still serves `h1=2` on every directory URL | Deploy, then re-crawl the six `/doctors` URLs and confirm `h1=1` |
...
| SEO-014 | Internal links with no anchor text on every page | Internal linking | **IMPLEMENTED LOCALLY — AWAITING DEPLOY** | 2026-08-19 | Whole-card overlay anchors (`absolute inset-0`, `tabIndex={-1}`) in `DoctorCard`, `ServiceCard` and `ServiceCatalog`, plus the cart and notifications icon links, carried only an `aria-label`. Crawlers read text content, so the FIRST link to every service and doctor URL in source order had empty anchor text. Converted to the repo's existing `sr-only` convention (already used in `ServiceCard`'s footer buttons and `CartServiceCard`); accessible names unchanged. The `/account` avatar's anchor text was the decorative glyph `•`/initial — now an `sr-only` label with the glyph `aria-hidden`. A scan of four production page types found 47 anchor-text-less internal links; the same scan against the patched build finds 0, and in-browser `/ireland/en` reports 0 with 69 `sr-only` anchors and no layout leak | Production still ships the empty anchors | Deploy, then re-run "Internal Outlinks With No Anchor Text" |
| SEO-015 | Hardcoded English section eyebrows on every localized public page | Content / i18n | **IMPLEMENTED LOCALLY — AWAITING DEPLOY** | 2026-08-19 | `ServiceIntro`'s `eyebrow` defaulted to the literal "Overview" and six routes passed literal `"Who it's for"`, `"Patient reviews"`, `"Trusted by patients"`, `"What patients say about"`. Confirmed live with a Googlebot UA: `/ireland/es`, `/portugal/cs`, `/brazil/es` **and `/spain/es`** — a market's own default locale — each rendered "Overview" 3× and "Who it's for"; `/spain/es/gp-consultation-online` and `/ireland/es/lab-tests` also rendered "Patient reviews" and "Trusted by patients". Not a fallback-locale problem: the surrounding CMS copy on those URLs is genuinely translated, which is why SEO-GROWTH-011's "locale variants are expected behaviour" verdict stands. New `common.sections.{overview,whoItsFor}` keys in all six locale bundles (phrasing lifted from the already-reviewed hub content), and the Doctify call sites now use the existing translated `a11y.patientReviews` / `doctify.patientsSayHeadline` / `patientsSayAccent` / `body`. **Copy change to note:** the review headlines lose their per-page specificity ("Trusted by patients in {country}" / "for prescriptions" / "across Europe") in favour of the one translated pair, because splitting a sentence across a country name breaks in inflected languages (CS "v Španělsko"). Local render: zero English leaks on five localized URLs, "Přehled"/"Descripción general" and the translated review headings present | Production still renders the English literals on every locale, including native-locale markets | Deploy, then re-fetch `/spain/es` and `/portugal/cs` and grep for "Overview" |
...
| SEO-I18N-001 | English insurance sentence in the meta description and hero of every localized service page | Content / i18n | **IMPLEMENTED LOCALLY — AWAITING DEPLOY** | 2026-08-25 | Found while investigating Portugal. `/portugal/pt/services/certificados-medicos` and `/services/baixa-medica` both served meta descriptions ending "We also have Medicare for this service." — an English sentence in a Portuguese SERP snippet. Root cause is `buildInsuranceSeoLine` in `backend/src/modules/services/services.service.ts`, hardcoded English by its own comment ("English-only for now"), consumed by the service page in **two** places: appended to the meta description *and* rendered as a visible green paragraph in the hero. Same defect class as SEO-015, which missed this call site. Fixed by composing the sentence in the frontend from `detail.insuranceOptions[].name` plus a new translated `serviceDetailPage.insuranceAvailability` template in all six locale bundles, joined with `Intl.ListFormat` — which matters beyond translation: the backend hardcoded " and ", where pt-PT uses "e", so even the conjunction was wrong. The backend field is left in place and simply no longer read by this page. New `lib/content/insurance-seo-line.ts` + 5/5 tests; `tsc --noEmit` clean. Note this only fires for services with a covering insurer, which is why it surfaced on two PT pages and not on the EN or CS spot checks | English sentence still served on every affected localized service page | Deploy, then re-fetch the two PT service URLs and confirm the description ends "Aceitamos também Medicare para este serviço." |
...
| SEO-404-001 | `llms.txt` advertised 6 URLs that 404 | Crawl efficiency / AI surfaces | **IMPLEMENTED LOCALLY — AWAITING DEPLOY** | 2026-08-25 | Found while triaging the GSC "Not found (404)" bucket (172 pages). Most of that bucket is retired Wix shapes and stale pre-redirect crawls — but Google was re-crawling current-shape URLs like `/czechia/en/lab-tests` as recently as 2026-08-22, and neither the sitemap nor any internal link or hreflang alternate points at them. Source was `app/llms.txt/route.ts`, which looped every country and pushed `gp-consultation-online`, `see-a-specialist` and `lab-tests` unconditionally. `app/sitemap.ts:480-499` had already fixed this exact bug behind `isCountryFeatureEnabled` (its comment at :471-476 describes the same failure); llms.txt never got the same gate. Live check confirmed **6 of 18 country links were 404** — CZ see-a-specialist + lab-tests, PT lab-tests, ES lab-tests, BR see-a-specialist + lab-tests. Fixed by applying the identical `specialist-consultations` / `health-tests` / `general-consultations` gates. **The gate alone was a no-op:** llms.txt read the static `@/data/countries`, where `enabledFeatures` is optional and never populated (0 occurrences), so `isCountryFeatureEnabled`'s "empty = all enabled" fallback passed everything. Route now awaits `getPublicCountriesMerged()`, the same backend-merged source sitemap.ts uses — free here, the route is already `force-dynamic` and already awaits the backend for the article list. Verified by running the frontend against the production API: regenerated llms.txt drops exactly those 6 lines, adds none, still lists all 6 countries, and all 12 remaining links return 200. `tsc --noEmit` clean | Dead links still served to AI crawlers | Deploy, then re-fetch `/llms.txt` and confirm 12 country links, zero 404s |
