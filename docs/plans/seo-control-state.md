...
| SEO-SPRAWL-001 | 523 URLs earning fewer than three impressions | Crawl budget | **BLOCKED — NEEDS A DECISION, NOT A PATCH** | 2026-08-24 | Mass-`noindex`ing locale variants is destructive and slow to reverse on Google's recrawl timescale. These URLs are live hreflang targets; de-indexing a variant that *is* serving a market costs that market. `SEO-GROWTH-011` already ruled locale variants expected behaviour. Deliberately not actioned unilaterally | n/a | Hassaan to decide per URL family whether the variant serves a real market before anything is de-indexed |
