...
- **4c. True SSR `lang` — DESIGN DONE, IMPLEMENTATION DEFERRED (2026-07-17).** Design pass hit the documented stop-condition. Findings: `[country]/[lang]/layout.tsx` already receives `lang` as a route param (no `rootParams()` needed) and could emit `<html lang>` — but only by becoming a Next root layout, which requires DELETING `app/layout.tsx` entirely (Next allows no partial opt-out). That cascades: `app/pay/`, `app/print/`, `app/share/`, `app/unauthorized/` have no group layouts and would each need one; and with no top-level layout, a truly-unmatched URL's branded 404 requires `app/global-not-found.tsx` behind `experimental.globalNotFound` — unstable Next surface for an already-mitigated issue. Deferred deliberately. If pursued: (1) fold pay/print/share/unauthorized into groups, (2) adopt experimental.globalNotFound with a staged monitoring pass, (3) split `(site)/(global)` + `(site)/[country]/[lang]` into root layouts. The shipped inline pre-hydration script remains the standing mitigation (sets lang before any content parses).
