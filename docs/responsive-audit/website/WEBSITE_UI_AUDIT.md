...
Companions: [`WEBSITE_SECTION_INVENTORY.md`](./WEBSITE_SECTION_INVENTORY.md), [`WEBSITE_OVERLAY_STACKING_INVENTORY.md`](./WEBSITE_OVERLAY_STACKING_INVENTORY.md), shared docs in `../shared/`.
...
Both High items depend on the shared AppMenu primitive (Phase 1/2 of `../shared/RESPONSIVE_IMPLEMENTATION_PLAN.md`); G1 can also ship standalone as an overflow-scope fix (smaller, earlier). i18n text fixes are independent, low-risk, immediately shippable.
