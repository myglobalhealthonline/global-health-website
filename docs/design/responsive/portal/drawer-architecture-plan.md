...
- One primitive: **`RecordDetailsDrawer`** built on the shared `AppSheet` (Radix Dialog side-variant; see `../shared/responsive-design-system-plan.md` §3.2). Same primitive later serves website filter sheets — no parallel implementations.
