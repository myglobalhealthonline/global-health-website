...
-- <=90 days, review invites in 14), so links issued before this migration
-- simply stop resolving instead of a dual-column backfill window — the
-- doctor/patient can re-request a fresh link. Uses Postgres's built-in
-- sha256(bytea) (no pgcrypto extension needed) so the hash matches the
-- Node `createHash("sha256").update(token).digest("hex")` used to look it
-- up, computed here for continuity of any live rows at deploy time.
