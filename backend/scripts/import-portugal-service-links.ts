/**
 * Seed the Portugal internal-link callout map (ServiceLink rows) from the
 * "Estrutura de Ligações Internas — Portugal" spec. Groups links by source
 * service and replaces that service's link set. Dry-run by default; --apply.
 *
 * Note: the PT spec assumes a catalogue that includes Saúde Mental (GP-level),
 * Saúde da Mulher, Saúde do Homem, Queda de Cabelo, Pediatria (Clínica Geral),
 * Segunda Opinião, and Referenciação e Exames Complementares — services that do
 * not exist in the live PT market. Only links whose source AND target map to a
 * real PT service are seeded here; the rest are reported as skipped.
 *
 *   node --env-file=.env --import tsx scripts/import-portugal-service-links.ts          # dry-run
 *   node --env-file=.env --import tsx scripts/import-portugal-service-links.ts --apply
 */
...
const DATA = path.join(here, "data", "portugal-service-links.json");
...
async function main() {
  ...
  console.log("\n────────────");
  if (skipped.length) {
    console.log("Skipped (no matching PT service):");
