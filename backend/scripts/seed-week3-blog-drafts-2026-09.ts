/**
 * Create the six Week 3 article sets (one per market) as production CMS drafts.
 *
 ...
 *
 * Usage from backend/:
 *   node --env-file=.env --import tsx scripts/seed-week3-blog-drafts-2026-09.ts
 *   node --env-file=.env --import tsx scripts/seed-week3-blog-drafts-2026-09.ts --apply
 */
...
const RESEARCHED_AT = "2026-09-07";
...
function validate(set: PostSet, post: LocalePost): string[] {
  ...
  if (prose < 900 || prose > 1_250) errors.push(`${label}: ${prose} prose words outside 900-1250`);
  const faqMinimum = 2;
  const faqMaximum = 4;
  ...
  // Only our own routes are blocked; external sources such as nhlbi.nih.gov/health/ are fine.
  if (/myglobalhealth\.online\/[a-z]+\/[a-z]{2}\/(?:blog\/categories|health)\//.test(body)) errors.push(`${label}: blocked internal route`);
  return errors;
}
...
async function main() {
  const sets = ONLY ? WEEK3_POST_SETS.filter((set) => set.key === ONLY) : WEEK3_POST_SETS;
  if (sets.length === 0) throw new Error(`--only=${ONLY} matched no Week 3 topic`);
  ...
  console.log(
    `${APPLY ? "APPLY" : "DRY RUN"}: ${sets.length} Week 3 topic(s), ` +
      `${sets.flatMap((set) => set.posts).length} primary-language draft(s)`,
  );
  ...
  for (const set of sets) {
    ...
    if (set.reviewerDoctorId) await verifyDoctor(set.reviewerDoctorId, country.id, "reviewer", set.key);
    else console.log(`${set.key}: WARNING no reviewerDoctorId — clinical reviewer must be assigned in the CMS before publication`);
