/**
 * Seed the Brazil internal-link callout map (ServiceLink rows) from the
 * "Documento de Ligações Internas — Brasil" spec (hub→spoke, 10 clinical
 * clusters, return-to-hub links). All Brazil GP services are GENERAL kind
 * (no specialist tier), so every link is typed COMPLEMENTARY.
 *
 * Requires the 18 Brazil services (see import-brazil-service-content.ts) to
 * exist first — links whose source or target slug isn't a real BR service
 * are skipped and reported. Dry-run by default; --apply.
 *
 *   node --env-file=.env --import tsx scripts/import-brazil-service-links.ts          # dry-run
 *   node --env-file=.env --import tsx scripts/import-brazil-service-links.ts --apply
 */
...
const DATA = path.join(here, "data", "brazil-service-links.json");
...
async function main() {
  ...
  for (const [sourceSlug, group] of bySource) {
    ...
    console.log(`${sourceSlug.padEnd(32)} ${resolved.length}/${group.length} links`);
  ...
  console.log("\n────────────");
  if (skipped.length) {
    console.log("Skipped (no matching BR service):");
