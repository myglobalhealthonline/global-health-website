/**
 * Seed the Spain internal-link callout map (ServiceLink rows) from the
 * "Documento de Enlazado Interno — España" spec (hub→spoke, clinical
 * clusters, second-opinion cluster, and GP↔specialist bidirectional pairs).
 * Groups links by source service and replaces that service's link set.
 *
 * Requires the 22 Spain services (see import-spain-service-content.ts) to
 * exist first — links whose source or target slug isn't a real ES service
 * are skipped and reported. Dry-run by default; --apply.
 *
 *   node --env-file=.env --import tsx scripts/import-spain-service-links.ts          # dry-run
 *   node --env-file=.env --import tsx scripts/import-spain-service-links.ts --apply
 */
...
const DATA = path.join(here, "data", "spain-service-links.json");
...
async function main() {
  ...
  console.log("\n────────────");
  if (skipped.length) {
    console.log("Skipped (no matching ES service):");
