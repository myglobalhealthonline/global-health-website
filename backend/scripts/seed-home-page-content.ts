...
// Pricing FAQ answer — used site-wide when a priced PUBLIC service was
// found; falls back to a no-number generic answer when it wasn't (e.g. br,
// which currently has no priced service — see cheapestPublicPriceLine).
// ponytail: one shared fallback instead of a market-specific branch — same
// text would otherwise apply to any market missing prices, not just br.
const PRICING_QUESTION: Record<LocaleCode, string> = {
  EN: "How much does a consultation cost?",
  CS: "Kolik stojí konzultace?",
  ES: "¿Cuánto cuesta una consulta?",
  RO: "Cât costă o consultație?",
  PT: "Quanto custa uma consulta?",
  DE: "Was kostet eine Konsultation?",
};

function pricingAnswer(locale: LocaleCode, priceLine: string | null): string {
  const withPrice: Record<LocaleCode, string> = {
    EN: `Consultations at Global Health cost ${priceLine}. There are no hidden fees and no membership required.`,
    CS: `Konzultace u Global Health stojí ${priceLine}. Žádné skryté poplatky, žádné povinné členství.`,
    ES: `Las consultas en Global Health cuestan ${priceLine}. Sin costes ocultos ni suscripción obligatoria.`,
    RO: `Consultațiile la Global Health costă ${priceLine}. Fără costuri ascunse, fără abonament obligatoriu.`,
    PT: `As consultas na Global Health custam ${priceLine}. Sem custos ocultos, sem subscrição obrigatória.`,
    DE: `Konsultationen bei Global Health kosten ${priceLine}. Keine versteckten Gebühren, keine Mitgliedschaft erforderlich.`,
  };
  const generic: Record<LocaleCode, string> = {
    EN: "Consultations at Global Health are priced individually by service — the exact price for your appointment type is shown clearly before you confirm your booking, so there are no surprises and no hidden fees.",
    CS: "Konzultace u Global Health mají individuální cenu podle typu služby — přesná cena za váš typ schůzky se zobrazí jasně před potvrzením rezervace, takže žádná překvapení ani skryté poplatky.",
    ES: "Las consultas en Global Health tienen un precio individual según el tipo de servicio — el precio exacto para su tipo de cita se muestra claramente antes de confirmar la reserva, sin sorpresas ni costes ocultos.",
    RO: "Consultațiile la Global Health sunt tarifate individual în funcție de tipul de serviciu — prețul exact pentru tipul dumneavoastră de programare este afișat clar înainte de a confirma rezervarea, fără surprize și fără costuri ascunse.",
    PT: "As consultas na Global Health têm um preço individual consoante o tipo de serviço — o preço exato para o seu tipo de consulta é apresentado claramente antes de confirmar a marcação, sem surpresas nem custos ocultos.",
    DE: "Konsultationen bei Global Health werden je nach Leistungsart individuell bepreist — der genaue Preis für Ihre Terminart wird vor der Buchungsbestätigung klar angezeigt, ohne Überraschungen und ohne versteckte Gebühren.",
  };
  return priceLine ? withPrice[locale] : generic[locale];
}

function faq(locale: LocaleCode, reg: string, priceLine: string | null): FaqItem[] {
  const c = CONNECTOR[locale](reg);
  const pricingItem: FaqItem = { question: PRICING_QUESTION[locale], answer: pricingAnswer(locale, priceLine) };
