...
import {readSpain as readSpainContent} from './spain-seo-storage.mjs';
...
const brazil=process.argv.includes('--brazil'),readSpain=brazil?readBrazil:readSpainContent;
const root=brazil?'seo/brazil':'seo/spain',read=p=>JSON.parse(fs.readFileSync(`${root}/${p}`));
const phase=Number(process.argv.find(a=>a.startsWith('--phase='))?.slice(8)??1);
const manifest=read(phase>1?`content-briefs/storage-mutation-manifest-phase${phase}.json`:'content-briefs/storage-mutation-manifest.json'),snapshot=read(phase>1?read(`content-briefs/phase${phase}-plan.json`).snapshot:'raw/storage-preflight-2026-09-13.json').data;
