...
export async function notifyAdminsOfInsuranceOrder(
  ...
  const emailHtml = wrapHtml("Insurance booking to verify", `
    ...
    <p><a href="${esc(verifyUrl)}" style="background:#1B4D3E;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;">Verify insurance</a></p>
...
export async function applyInsuranceVerificationDecision(
  ...
  if (decision === "VERIFIED") {
    const html = wrapHtml("Your insurance was verified", `
      ...
      <p><a href="${esc(payLink)}" style="background:#1B4D3E;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;">Pay &amp; confirm</a></p>
    ...
    const html = wrapHtml("We couldn't verify your insurance card", `
      ...
      <p><a href="${esc(payLink)}" style="background:#1B4D3E;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;">Book at standard price</a></p>
      <p style="font-size:12px;color:#737373;">${esc(payLink)}</p>
    `);
