...
async function dispatch(to: string, locale: LocaleCode, built: BuiltEmail): Promise<void> {
  const bodyHtml = [
    ...
    built.cta
      ? `<p style="margin:24px 0;text-align:center;"><a href="${built.cta.href}" style="background:#B0F122;color:#0a1f14;padding:13px 24px;border-radius:999px;text-decoration:none;font-weight:700;display:inline-block;">${escapeHtml(built.cta.label)}</a></p>`
      : "",
  ].join("\n");
