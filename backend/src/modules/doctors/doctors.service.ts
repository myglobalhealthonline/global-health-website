...
/* ------------------------------------------------------------------ *
 * Public doctor-card projection (perf plan docs/plans/new.md 7.1)
 * ------------------------------------------------------------------ */

/** One card row. Raw-payload-shaped on purpose: the frontend keeps its
 *  existing `CountryDoctorCard` mapper unchanged, so parity is by
 *  construction. `additionalCountries` / `editorialChecklist` are emitted as
 *  one-field shims because that mapper reads the chamber and the
 *  non-physician flag out of those nested shapes. */
export type PublicDoctorCard = {
  ...
  imcRegistration: string | null;
  additionalCountries: Array<{ chamberEntity: string | null }>;
  registrationDivision: string | null;
  registrationVerified: boolean;
  credentials: Array<{ label: string; bodyName: string; bodyUrl: string | null }>;
  medicalRegistrationUrl: string | null;
  instagramUrl: string | null;
  facebookUrl: string | null;
  linkedinUrl: string | null;
  isFeatured: boolean;
  editorialChecklist: { nonPhysician: true } | null;
  bookability: BookabilitySummary;
  bookabilityByServiceId: Record<string, BookabilitySummary>;
};

function readNonPhysician(checklist: Prisma.JsonValue | null | undefined): boolean {
  if (!checklist || typeof checklist !== "object" || Array.isArray(checklist)) return false;
  return (checklist as Record<string, unknown>).nonPhysician === true;
}

/**
 * Card-only variant of `listDoctorsByCountry`. Same `where`, `orderBy`,
 * `take`, per-market registration override, credential country filter,
 * featured-doctor lookup and bookability payload -- only the selected columns
 * and emitted keys are narrowed to what a doctor card renders. FAQs, SEO
 * fields, qualifications, raw translation rows and the country object are
 * dropped; `whatsappNumber` and the booking-pause fields are never selected,
 * so there is nothing for `stripPrivateContact` to strip here.
 *
 * The legacy endpoint is untouched: it stays the runtime fallback and the
 * permanent source for `getPublicDoctorsForMarket` (sitemap/hreflang).
 */
export async function listDoctorCardsByCountry(
  countryCode: string,
  locale?: LocaleCode,
): Promise<PublicDoctorCard[]> {
  try {
    const rows = await timePhase("query", () => prisma.doctor.findMany({
      where: {
        active: true,
        OR: [
          { country: { code: countryCode, isActive: true } },
      ...
      select: {
        ...
        additionalCountries: {
          ...
          select: {
            ...
            isVerified: true,
            translations: { select: { locale: true, title: true, bio: true } },
            divisionTranslations: { select: { locale: true, division: true } },
          },
          take: 1,
        },
        credentials: {
    ...
    return rows.map((row) => {
      ...
      // Same two-stage resolve as mergeDoctorTranslation +
      // mergeDoctorMarketTranslation: a market row only wins when it resolved
      // to the SAME locale as the doctor-level row, otherwise a
      // default-locale market row stomps a correct per-locale bio.
      const doctorTr = resolveTranslation(row.translations, requested, defaultLocale);
      const marketTr = resolveTranslation(market?.translations ?? [], requested, defaultLocale);
      const applyMarket = marketTr.resolvedLocale === doctorTr.resolvedLocale;
      const title = (applyMarket ? marketTr.tr?.title : null) ?? doctorTr.tr?.title ?? row.title;
      const bio = (applyMarket ? marketTr.tr?.bio : null) ?? doctorTr.tr?.bio ?? row.bio;
      const { tr: divisionTr } = resolveTranslation(
        market?.divisionTranslations ?? [],
        requested,
        defaultLocale,
      );
      return {
        ...
        specialties: row.specialties.map(({ specialty }) => {
          const { tr } = resolveTranslation(specialty.translations, requested, defaultLocale);
          return { specialty: { name: tr?.name ?? specialty.name } };
        }),
        ...
        assignedServices: row.assignedServices.map((a) => ({ serviceId: a.serviceId })),
        imcRegistration: market?.registrationNumber ?? null,
        additionalCountries: market ? [{ chamberEntity: market.chamberEntity }] : [],
        registrationDivision: divisionTr?.division ?? market?.division ?? null,
        registrationVerified: market?.isVerified ?? false,
        credentials: row.credentials
          .filter((c) => !c.countryCode || c.countryCode.toUpperCase() === code)
          .map((c) => ({ label: c.label, bodyName: c.bodyName, bodyUrl: c.bodyUrl })),
        medicalRegistrationUrl: market?.registrationUrl ?? row.medicalRegistrationUrl ?? null,
        instagramUrl: row.instagramUrl,
        facebookUrl: row.facebookUrl,
        linkedinUrl: row.linkedinUrl,
        isFeatured: row.id === featuredId,
