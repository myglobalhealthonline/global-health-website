  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // asking for a wider window (the admin's month picker, or a longer
        // `days`) can legitimately reach real open slots beyond that horizon —
        // that's RETURNING + NO_OPEN_SLOT (nextAvailableAt points at that later
        // slot). UNAVAILABLE + NO_OPEN_SLOT is different: nothing is open even
        // in the wider lookahead, so — like any pause/no-doctor reason — there
        // is truly nothing to list regardless of the requested range.
