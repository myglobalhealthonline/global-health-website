...
export function buildReviewInviteEmail(opts: {
  ...
  return {
    ...
    html: wrapHtml(title, `<div style="font-family:Arial,sans-serif;font-size:16px;line-height:1.6;color:#143529;"><p>${escapeHtml(body)}</p>${button}<p style="font-size:14px;color:#52645B;">${escapeHtml(copy.privacy)}</p><p style="font-size:14px;"><a href="${link}" style="color:#52645B;text-decoration:underline;">${escapeHtml(copy.alreadyReviewed)}</a> · <a href="${link}" style="color:#52645B;text-decoration:underline;">${escapeHtml(copy.optOut)}</a></p></div>`),
  };
}
