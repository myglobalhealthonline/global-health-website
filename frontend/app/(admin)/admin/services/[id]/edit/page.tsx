...
export default async function AdminEditServicePage({ params, searchParams }: PageProps) {
  ...
  if (!serviceResult.ok) {
    return (
      <section className="gh-card p-6 sm:p-8">
        <h1 className="gh-h2 text-[var(--color-text-primary)]">Edit service</h1>
        <p className="mt-4 text-amber-900">Could not load service: {serviceResult.message}</p>
  ...
  if (!specialtiesResult.ok) {
    return (
      <section className="gh-card p-6 sm:p-8">
        <h1 className="gh-h2 text-[var(--color-text-primary)]">Edit service</h1>
        <p className="mt-4 text-amber-900">Could not load specialties: {specialtiesResult.message}</p>
      </section>
    );
  }
