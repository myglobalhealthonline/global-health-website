  ...
  if (!countryId) {
    return (
      <section className="gh-card p-6 sm:p-8">
        ...
        <Link href={meta.listHref} className="mt-6 inline-block gh-link text-[var(--color-brand-primary)]">
  ...
  return (
    <section className="gh-card p-6 sm:p-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="gh-h2 text-[var(--color-text-primary)]">New {meta.singularLabel.toLowerCase()}</h1>
        <Link href={`${meta.newHref}?kind=${encodeURIComponent(kind)}`} className="gh-link text-sm text-[var(--color-text-muted)]">
          Change country
        </Link>
      </div>
