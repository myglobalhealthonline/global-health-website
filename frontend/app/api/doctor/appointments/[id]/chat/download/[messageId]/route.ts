 ...
 * The chat message's `downloadUrl` is a bare `/api/doctor/...` path (no
...
export async function GET(
  ...
  const upstream = await fetch(
    `${backend}/api/doctor/appointments/${encodeURIComponent(id)}/chat/download/${encodeURIComponent(messageId)}`,
    {
      method: "GET",
