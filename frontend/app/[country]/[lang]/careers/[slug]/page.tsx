...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const path = `/${value.country}/${value.lang}/careers/${result.job.slug}`;
...
export default async function JobDetailPage({ params }: { params: Promise<Params> }) {
  ...
  if (result.state === "unavailable") return <main className="gh-careers-detail gh2-section-ivory">
    <div className="mx-auto max-w-[var(--container-width)] px-5 py-24 md:px-10">
      ...
      <div className="gh-careers-state" role="status"><strong>{context.t.unavailableTitle}</strong><p>{context.t.unavailableBody}</p></div>
    </div>
  </main>;
  if (result.state !== "loaded") notFound();

  const job = result.job;
  const descriptionHtml = sanitizePageBodyHtml(job.descriptionHtml);
  const relativeUrl = `${base}/careers/${job.slug}`;
  const canonicalUrl = `${getSiteUrl()}${relativeUrl}`;
  const workplaceLabel = context.t[`workplace${job.workplaceMode}` as "workplaceREMOTE" | "workplaceHYBRID" | "workplaceONSITE"];

  return <main className="gh-careers-detail gh2-section-ivory">
    ...
    <div className="mx-auto max-w-[var(--container-width)] px-5 py-12 md:px-10 md:py-20">
      <Link href={`${base}/careers`} className="gh-careers-back"><ArrowLeft className="size-4" aria-hidden />{context.t.backToOpenings}</Link>
      <div className="gh-careers-detail-grid">
        <article>
          <header className="gh-careers-job-header"><p>{job.department} · {job.location}</p><h1>{job.title}</h1></header>
          <div className="gh-careers-prose" dangerouslySetInnerHTML={{ __html: descriptionHtml }} />
          <JobApplicationForm jobId={job.id} privacyHref={`${base}/legal/privacy-policy`} copy={{
        ...
        <aside className="gh-careers-job-aside">
          ...
          <dl>
            <div><dt><MapPin aria-hidden />{context.t.locationLabel}</dt><dd>{job.location}</dd></div>
            <div><dt><Building2 aria-hidden />{context.t.departmentLabel}</dt><dd>{job.department}</dd></div>
            <div><dt><BriefcaseBusiness aria-hidden />{context.t.employmentTypeLabel}</dt><dd>{job.employmentType}</dd></div>
            <div><dt><Clock3 aria-hidden />{context.t.workplaceLabel}</dt><dd>{workplaceLabel}</dd></div>
            {job.minimumExperience ? <div><dt>{context.t.experienceLabel}</dt><dd>{job.minimumExperience}</dd></div> : null}
          </dl>
        </aside>
      </div>
    </div>
