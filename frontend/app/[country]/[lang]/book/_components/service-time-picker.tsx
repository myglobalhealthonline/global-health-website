...
type Props = {
   ...
   *  the DOCTOR step, or its own re-fetch falls back to the default (current)
...
export function ServiceTimePicker({
  ...
  function chooseTime(startAt: string) {
    ...
    startNavigate(() => {
      router.push(
        `${buildBookHref({ country, lang, service: serviceSlug, benefit, at: startAt, month: String(month) })}#booking`,
      );
    });
  }
