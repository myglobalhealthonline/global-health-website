...
/**
 * The public shell for routes with NO country/lang URL segment: providers,
 * consent-gated analytics, `SiteChrome` (header, footer, trust bar) and the
 * site-wide JSON-LD. It reads headers()/cookies() to guess a "last known
 * country" for the header/footer IA, so every caller is request-time
 * rendered — fine here: this is not the SEO-critical bulk of the site, and
 * being dynamic is precisely why the resolved locale can go straight into
 * `<html lang>` with no inline correction script.
 *
 * Extracted from `app/(global)/layout.tsx` so the 404 boundaries can reuse
 * the exact same chrome instead of growing a second copy that drifts. Two
 * callers need the document too (`(global)` layout, `global-not-found`) and
 * one does not (`(redirect)/not-found`, whose own root layout already
 * rendered `RootDocument`) — hence `withDocument`.
 */
export async function PublicShell({
  ...
  const [{ common, navigation }, assets, activeFooter, activeTrust, reviewConfigResult] =
    await Promise.all([
      // Same locale the rest of this shell renders in — passing the raw
  ...
  // footers for other countries, so requesting all 5 every render would
  // burn 4 round-trips for data nothing reads. Outside a country scope
  // (entry gate / global pages) the fetch is skipped and SiteFooter falls
  ...
  if (!withDocument) return shell;
  return <RootDocument lang={toHtmlLang(currentLocale)}>{shell}</RootDocument>;
}
