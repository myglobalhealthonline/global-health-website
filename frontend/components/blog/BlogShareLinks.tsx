...
function ShareAnchor({ href, label, children }: ShareAnchorProps) {
  return (
    <a
      href={href}
      target="_blank"
...
export function BlogShareLinks({ articleUrl, title }: BlogShareLinksProps) {
  ...
  const copyLink = async (forInstagram = false) => {
    try {
      ...
      setNotice(forInstagram ? "Link copied — open Instagram to share it." : "Article link copied.");
    } catch {
      setNotice("Copy failed. Select the address from your browser instead.");
    }
  };
  ...
  return (
    <section className="gh-blog-calm-share" aria-labelledby="share-this-article">
      <div className="gh-blog-calm-share-inner">
        <div>
          <p className="gh-blog-calm-share-kicker">Worth sharing?</p>
          <h2 id="share-this-article">Share this article</h2>
        </div>
        <div className="gh-blog-calm-share-actions">
          <ShareAnchor href={links.facebook} label="Facebook">
          ...
          <ShareAnchor href={links.whatsapp} label="WhatsApp">
          ...
          <ShareAnchor href={links.x} label="X">
          ...
          <ShareAnchor href={links.linkedin} label="LinkedIn">
          ...
          <button type="button" className="gh-blog-share-action" onClick={() => copyLink()}>
            {notice.startsWith("Article link copied") ? <Check aria-hidden /> : <Copy aria-hidden />}
            <span>Copy link</span>
          </button>
        </div>
