...
import {languageNames,booking,missingFaqs,dermAnswers,newFaqCopy,gpCopy,faqOverrides} from './content-briefs/copy.mjs';
...
// Owner attestations (2026-09-14, ledger §56.5). They lift holds; they are not registry verification.
const ownerAttested={'dr-luz-marina-zuluaga-rios':'Owner confirmed 2026-09-14 she is currently practising; CGCOM recheck 2026-09-14 still ALTA sin ejercicio','dr-tomas-ruiz-palacios':'COP Región de Murcia public search 2026-09-14: TOMAS RUIZ PALACIOS, MU05691 (stored MUO5691 is a letter-O typo)','dr-leandro-wang':'Owner confirmed 2026-09-14 specialist qualification evidence exists; source not yet on file'};
const esDescriptions={'consulta-diagnotico-vascular':'Revisión especializada de tu eco-Doppler, índice tobillo-brazo o angio-TC por videollamada en España. Interpretación, plan y solicitud de pruebas.','consulta-flebologia-y-linfologia':'Consulta especialista online en varices, insuficiencia venosa, linfedema y lipedema en España. Revisión de pruebas, compresión indicada y plan de tratamiento.','consulta-online-medicina-estetica':'Medicina estética online con médica colegiada en España. Valoración independiente antes de tratarte: indicación, expectativas realistas, riesgos y plan escrito.','consulta-salud-vascular-circulatoria':'Piernas pesadas, varices, hinchazón o calambres. Valoración médica online de tu salud venosa en España: revisión de pruebas, solicitud de eco-Doppler y plan.'};
const bodies=fs.existsSync(`${root}/content-briefs/body-localization.json`)?read('content-briefs/body-localization.json'):{};
...
for(const p of pages.filter(p=>p.status===200&&['service','doctor'].includes(type(p)))){
 ...
 if(type(p)==='service'){
  ...
  if(s.resolvedLocale===p.locale.toUpperCase()){
   ...
   if(missingFaqs[s.slug]){
    ...
    addedFaqs=(p.locale==='es'?missingFaqs[s.slug]:newFaqCopy[p.locale].filter((_,i)=>s.slug!=='consulta-salud-vascular-circulatoria'||i!==1).map(([q,a],i,arr)=>[q,faqOverrides[s.slug]?.[p.locale]?.[i]??a]).map(([q,a],i,arr)=>[q,i===arr.length-1?a+actualLanguages+'.':a])).map(([question,answer])=>({question,answer}));reasons.push('No native or embedded FAQs; answer procedure limits, preparation and actual language. New ES base and translations in existing locales; no new URL.');
    const langs=s.slug==='consulta-salud-vascular-circulatoria'?'español, inglés, húngaro o alemán':'español';
    // Non-ES records store null and render the ES body; never copy Spanish into them. Use a localized draft or leave the fallback.
    const localized=p.locale==='es'?null:bodies[s.slug]?.[p.locale];
    if(localized)for(const key of ['seoTitle','seoDescription','heroTitle']){before[key]=s[key];after[key]=localized[key];}
    else if(p.locale==='es'&&esDescriptions[s.slug]){assert(esDescriptions[s.slug].length<=160);before.seoDescription=s.seoDescription;after.seoDescription=esDescriptions[s.slug];reasons.push('Remove unconditional same-day promise (availability is dynamic) and fit 160 characters.');}
    for(const key of ['heroDescription','detailBody']){let v=s[key];if(!v)continue;v=v.replaceAll('español, inglés o portugués',langs).replaceAll('inglés o portugués',langs).replace('disponible el mismo día desde cualquier punto de España y también en español','disponible desde cualquier punto de España, en español');if(key==='heroDescription'&&p.locale!=='es')v=booking[p.locale](actualLanguages);if(key==='detailBody'&&p.locale!=='es'){if(!localized)continue;v=localized.detailBody;}if(v!==s[key]){before[key]=s[key];after[key]=v;reasons.push(`${key}: unsupported consultation language corrected against assigned doctor. Longer Spanish clinical body in non-ES records remains a separate localization hold.`);}}
   }
 ...
 if(Object.keys(after).length||faqPatches.length||addedFaqs.length){
  const luzHold='CGCOM rechecked 2026-09-14: ALTA sin ejercicio, contradicting owner attestation; registry update or documented explanation required';
  const hold=s.slug==='dr-luz-marina-zuluaga-rios'?luzHold:type(p)==='service'&&s.assignedDoctorIds.some(id=>roster.some(d=>d.id===id&&d.slug==='dr-luz-marina-zuluaga-rios'))?'Assigned clinician Luz: '+luzHold:s.bookability.state!=='BOOKABLE'?'No current open appointment; retain routing and assignments':'';
  const d={key:`${type(p)}:${s.slug}`,url:p.url,affectedUrls:pages.filter(x=>x.status===200&&new URL(x.url).pathname.split('/')[4]===s.slug).map(x=>x.url),slug:s.slug,id:s.id,locale:p.locale,resolvedLocale:s.resolvedLocale,source:response.source,sourceFingerprint:hash(s),before,after,faqPatches,addedFaqs,removedEmbeddedSection:'',reasons,hold,status:'local candidate; unapproved; not applied'};d.payloadSha256=hash(d);drafts.push(d);
 }
}
...
const matrix=pages.map(p=>{
 ...
 const bodySlug=Object.keys(missingFaqs).find(slug=>p.url.endsWith('/'+slug));
 if(p.locale!=='es'&&bodySlug)reason+=bodies[bodySlug]?.[p.locale]?' Non-ES record stores no body and renders the ES body; localized title, description, H1 and body candidate prepared from the corrected ES source. Clinical and native-language review required.':' Non-ES record stores no body and renders the ES body; body localization held with the service. Clinical source and native-language review required.';
...
const facts=roster.map(d=>{const r=registry.records.find(r=>r.slug===d.slug);return {doctor_id:d.id,name:d.fullName,slug:d.slug,stored_number:d.imcRegistration,official_number:r?.number??'',official_name:r?.name??'',official_specialty:r?.specialty??'',official_status:r?.status??'unverified',verification_status:d.slug==='dr-luz-marina-zuluaga-rios'?'discrepancy: registry ALTA sin ejercicio (rechecked 2026-09-14) vs owner attestation':d.slug==='dr-tomas-ruiz-palacios'?'named registry record observed: '+ownerAttested[d.slug]:r?'named registry record observed; biography not certified':'unverified',consultation_languages:d.languages.join(' | '),bookability:d.bookability.state,source:r?.source??d.medicalRegistrationUrl,scope_hold:d.slug==='dr-leandro-wang'?ownerAttested[d.slug]+'; registry label Médico General':d.slug==='dr-silvina-irale'?'Registry Médico General; do not promote as specialist pediatrician':d.slug==='dr-tomas-ruiz-palacios'?'Verified against COP Murcia 2026-09-14 (raw/registry-recheck-tomas-2026-09-14.json); correct stored number typo MUO5691 to MU05691 via the doctor portal':'Retain detailed biography claims; not independently verified'};});
