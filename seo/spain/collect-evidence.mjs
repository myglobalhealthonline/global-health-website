...
const market = process.argv.includes('--brazil') ? 'brazil' : 'spain';
const countryCode = market === 'brazil' ? 'br' : 'es';
const root = `seo/${market}/raw`, origin = 'https://www.myglobalhealth.online';
...
const urls = [...new Set([...sitemap.html.matchAll(/<loc>(.*?)<\/loc>/g)].map(m => m[1]).filter(u => u.startsWith(`${origin}/${market}/`)))];
  ...
  await Promise.all(Array.from({ length: 4 }, async () => {
    while (next < batch.length) {
      ...
      try {
        ...
        pages.push({ ...parse(r.html, url), discoveryLinks: [...r.html.matchAll(/<a\b[^>]*\bhref="([^"]+)"/gi)].map(m=>({href:m[1].replaceAll('&amp;','&')})), status: r.status, finalUrl: r.finalUrl, checkedAt: r.checkedAt, discovery });
      } catch (e) { pages.push({ url, discovery, error: e.message }); }
      if (pages.length % 40 === 0) console.log(`Collected ${pages.length} ${market} URLs`);
    }
  }));
}
...
for (;;) {
  const linked = [];
  for (const p of pages) for (const a of p.discoveryLinks ?? p.links ?? []) {
    let u; try { u = new URL(a.href, p.url); } catch { continue; }
    if (u.origin !== origin || !u.pathname.startsWith(`/${market}/`) || /\.(pdf|png|jpg|svg)$/i.test(u.pathname)) continue;
...
for (const p of pages) {
  ...
  for (const k of ['doctors','services']) endpoints.set(`${k}-${locale.toUpperCase()}`, `/api/countries/${countryCode}/${k}?locale=${locale.toUpperCase()}`);
  if (['doctors','services'].includes(kind) && slug) endpoints.set(`${kind}-${slug}-${locale.toUpperCase()}`, kind === 'services' ? `/api/services/${slug}?countryCode=${countryCode}&locale=${locale.toUpperCase()}` : `/api/countries/${countryCode}/doctors/${slug}?locale=${locale.toUpperCase()}`);
}
