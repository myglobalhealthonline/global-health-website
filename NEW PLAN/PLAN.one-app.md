...
**Document version:** 1.1 — one-app architecture update; supersedes all prior stage documents
...
7. [Admin area specification](#7-admin-portal-specification)
...
A scalable global healthcare platform that connects patients with licensed doctors through online consultation requests. Patients pick a country, choose a service, and submit an appointment request. Admins manage every country, doctor, service, website page, and appointment request from a single protected super-admin area inside the same website.
...
### 1.2 Product structure — one website, protected admin area

This is **one Next.js application**, not two separate applications.

The same codebase serves:

- **Public website** at `www.myglobalhealth.online`
- **Super admin area** at `www.myglobalhealth.online/admin`
- **Shared database** for countries, doctors, services, categories, admin users, and appointment requests

The admin area is protected by authentication and middleware. Public visitors never see admin screens, and admin routes never expose private data without a valid admin session.

### 1.3 Long-term user types

The platform has three long-term user types:

1. **Super Admin / Admin**
   - Logs into `/admin`.
   - Manages countries, website content, services, doctors, categories, admin users, and appointment requests.
   - This is the only fully authenticated user type built in v1.

2. **Patient**
   - Uses the public website.
   - Can browse countries, doctors, and services.
   - Can submit an appointment request in v1.
   - Patient login, patient dashboard, booking history, prescription requests, and self-managed bookings are deferred to v2.

3. **Doctor**
   - Appears on the public website as a managed profile in v1.
   - Doctors do not log in during v1.
   - Doctor portal, schedule management, clinical workflows, notes, prescriptions, and patient messaging are deferred to a later separate project.

### 1.4 Scope — v1 (current build)

- **One public website** at `www.myglobalhealth.online`
- **Protected super-admin area** at `www.myglobalhealth.online/admin`
...
- Appointment intake form (writes to `Appointment` table; no payment and no real calendar scheduling yet)
- Admin appointment review/status management
...
- **Doctor portal** with clinical workflows — separate future project, not built here
- **Email transactional flows** (reset password, booking confirmation) — placeholder/manual in v1, full implementation v2
- **Multilingual auto-translation** — each country shows content in its primary language as entered by admin; auto-translation is not in scope
- **Real time-slot booking system** — v2
- **Video consultation room** — future scope
- **Prescription workflow** — future scope
- **Doctor clinical notes** — future scope

### 1.6 Success criteria for v1

- Admin can add a new country end-to-end (name, hero, languages, currency) and have it appear on the public website within minutes without a deploy
- Admin can publish a new service in any country, with category, price, doctor assignments, and image
- Admin can review appointment requests submitted from the public website
- All 200+ existing Wix URLs continue to resolve (no SEO loss)
- Public website Lighthouse scores: Performance 90+, Accessibility 95+, SEO 95+
- Brand identity (Global Health green palette, Gilroy typography) applied consistently across public and admin areas
...
Gilroy is a commercial typeface — ensure you have the appropriate license (it's available from Type Type Foundry). Self-host the woff2 files in `public/fonts/` for public and admin areas; do not load from a CDN you don't control.
...
- **Public website header** (white background): Full color on white
- **Public website footer** (Forest background): All white
...
Store the logo as SVG in `apps/web/public/logos/` and `apps/web/public/logos/`. Provide a horizontal lockup (logo + wordmark inline) for tight header spaces.
...
- **One Next.js app, not two apps** — the business requirement is one scalable website with a protected admin area. Keeping public pages and admin routes in one app reduces setup complexity, avoids duplicated configuration, and makes Phase 0 faster. Admin remains isolated through route structure, middleware, role checks, and server-side authorization.
- **Admin under `/admin`** — the canonical admin URL is `www.myglobalhealth.online/admin`. A future `admin.myglobalhealth.online` subdomain can redirect to `https://www.myglobalhealth.online/admin` if desired, but it should not require a second app.
- **Prisma owns the schema in `packages/db/`** — single source of truth. The app imports the generated client through the shared database package.
...
- **Railway for everything** — Postgres, object storage, app deploy, all in one provider. Simpler ops than splitting across Vercel/Supabase/Cloudflare for v1.
...
├── apps/
│   └── web/                       Single Next.js app → public website + protected admin
│       ├── app/
│       │   ├── (site)/            Public marketing pages
│       │   ├── [country]/         Country-scoped public routes
│       │   ├── admin/             Protected super-admin area
│       │   │   ├── login/
│       │   │   ├── countries/
│       │   │   ├── categories/
│       │   │   ├── doctors/
...
### 4.4 Public website URL structure
...
The admin lives inside the same website under `/admin`.

```
/admin/login                       Public login page
/admin                             Dashboard (auth required)
/admin/countries                   List of countries
/admin/countries/[id]              Edit country
/admin/categories                  Global categories + country matrix
/admin/categories/[id]             Edit category
/admin/doctors                     All doctors
/admin/doctors/new
/admin/doctors/[id]                Edit doctor (M:N country assignments)
/admin/users                       Manage admins
/admin/audit                       Audit log
/admin/settings                    Account settings

# Country-scoped admin routes:
/admin/[country]                   Country dashboard
/admin/[country]/content           Edit hero, contact, languages
/admin/[country]/general           General consultations list
/admin/[country]/general/[id]
/admin/[country]/specialist
/admin/[country]/specialist/[id]
/admin/[country]/prescriptions
/admin/[country]/prescriptions/[id]
/admin/[country]/health-tests
/admin/[country]/health-tests/[id]
/admin/[country]/appointments      Appointment requests
```

The active country lives in the URL (single source of truth). The country dropdown in the topbar rewrites the `/admin/[country]/...` segment when changed. Cookie stores the last-selected country to default new sessions back to it.


### 4.6 Cache invalidation inside the one app

Because public pages and admin routes live in the same Next.js app, admin mutations do not need to call a separate deployed app to refresh content.

Pattern: admin server actions, after a successful DB write, call `revalidateTag()` for the affected public data tags. Within seconds the public pages rebuild with the latest database content.
...
A separate `/api/revalidate` endpoint is optional for future external integrations, but it is not required for v1 admin-to-public updates because both live in one app.
...
| **Appointment** | Appointment intake/request record (no payment and no confirmed calendar slot in v1) | `serviceId`, `countryId`, `patientName/Email/Phone`, `status`, `notes` |

For v1, `Appointment` means a submitted request that admins can review and manage. It is not a full scheduled booking with patient self-service. Real calendar booking is deferred to v2.
...
- **DRAFT**: Visible only in admin. Public website queries filter `status = PUBLISHED`.
...
### 6.1 Public website pages — static (handcrafted)
...
### 6.2 Public website pages — dynamic (DB-driven)
...
## 7. Admin area specification

### 7.1 Authentication

- **Login at** `/admin/login`. Email + password, server-rendered form.
- **Session**: JWT signed with `jose`, stored in httpOnly secure cookie.
- **Middleware** at `apps/web/middleware.ts` redirects unauthenticated `/admin/*` requests to `/admin/login`, with `?next=` for post-login redirect.
...
Every admin page (except `/admin/login`) renders inside this shell:
...
**Environment variables (`apps/web/.env.local` and production app env):**
```
...
- Use Next.js `<Image>` component everywhere on the public website.
...
**Goal**: Repo structured, brand system in place, the single app boots with public and admin placeholders.

- ☐ Restructure repo to `apps/web` + `packages/*`
- ☐ Bootstrap `apps/web` as a fresh Next.js 16 + Tailwind 4 + shadcn app
- ☐ Create public placeholder page at `/`
- ☐ Create protected-admin placeholder structure under `/admin` (auth can be mocked until Phase 1)
- ☐ Install Gilroy font files in `apps/web/public/fonts` if licensed; otherwise use fallback font temporarily
- ☐ Create `packages/design-tokens` with the brand CSS variables (Forest scale, Lime scale, neutrals)
- ☐ Apply tokens to `apps/web/tailwind.config.ts` via a shared preset
- ☐ Create brand logo SVGs in `apps/web/public/logos/`
- ☐ Update `pnpm-workspace.yaml` and root `package.json`
- ☐ The app renders a public placeholder page and an admin placeholder page in brand colors
- ☐ Document the design system in `docs/BRAND.md`

**Acceptance**: `pnpm --filter web dev` shows a Forest-green public welcome page at `/` and a Forest-green admin placeholder at `/admin`. No errors.
...
- ☐ Create the initial schema migration for the clean build
- ☐ Seed 5 countries, core categories, and the first super admin user
...
- ☐ Build login page in `apps/web/app/admin/login/page.tsx`
- ☐ Build `apps/web/middleware.ts` — JWT verification, redirect unauthenticated `/admin/*` requests to `/admin/login`
- ☐ Build server actions: `login`, `logout`
- ☐ Test login flow with seeded super admin credentials

**Acceptance**: Super admin can log in at `/admin/login`. Unauthenticated requests to `/admin/*` redirect to `/admin/login`. JWT cookie set securely. Audit log row created on login.
...
- ☐ Build `/admin/countries` list page (table, search, drag-to-reorder via dnd-kit)
- ☐ Build `/admin/countries/[id]` edit form (metadata, hero, contact, deactivate)
...
**Acceptance**: Super admin can create a 6th country, edit it, toggle active, see it in the list. All actions appear in audit log. Public website doesn't crash (still reading from static data — that comes in Phase 5).

### Phase 3 — Categories and Doctors (4 days)

**Goal**: Admin can manage the global category pool and the M:N doctor roster.

- ☐ Build `/admin/categories` matrix view (rows = categories, cols = countries, cells = toggle)
- ☐ Build category create/edit drawer
- ☐ Wire toggle to `CategoryCountry` upserts/deletes (optimistic UI)
- ☐ Build `/admin/doctors` list page with country filter
- ☐ Build `/admin/doctors/[id]` edit form (split layout)
...
- ☐ Build `/admin/[country]/general` list page
- ☐ Build `/admin/[country]/specialist` list page (mostly identical config)
- ☐ Build `/admin/[country]/prescriptions` list page (no category dropdown)
- ☐ Build `/admin/[country]/health-tests` list page (no category dropdown)
- ☐ Build `/admin/[country]/[type]/[id]` edit pages — all four route to the same form, just preset the type
...
### Phase 5 — Public website goes dynamic (5 days)

**Goal**: Public website reads from DB. Replaces `data/*.ts` files.
...
- ☐ Update admin server actions to call `revalidateTag()` after every successful mutation
- ☐ Build legacy URL redirect map in `next.config.ts` (`/home-pt` → `/pt`, etc.)
- ☐ Apply brand styling to all pages (this is the visual launch)

**Acceptance**: Public website renders fully from DB. Admin publishes a change, public page updates within 30 seconds. All legacy URLs resolve (200 or 301).
...
- ☐ Build `/admin/[country]/content` page
- ☐ Form covers hero title/subtitle/CTA, contact phone/email/whatsapp, languages
- ☐ Verify public website reflects changes after publish
- ☐ Build admin areaointments page (`/admin/[country]/appointments`) — list, view, change status
...
- ☐ Test mobile responsive for public website (phone primary)
...
- ☐ Set up production Railway environment (Postgres, one app service, object storage)
- ☐ Configure custom domains: `www.myglobalhealth.online` and `www.myglobalhealth.online/admin`
...
**Acceptance**: `www.myglobalhealth.online` and `www.myglobalhealth.online/admin` both serve from Railway. Old Wix site can be turned off. No SEO regressions in first 7 days.
...
- `www.myglobalhealth.online/admin` → protected admin area inside the same deployment
- `admin.myglobalhealth.online` → optional redirect to `https://www.myglobalhealth.online/admin` if the business wants the subdomain later
...
|---|---|---|
| `gh-postgres` | Database | Shared Postgres for public content, admin users, and appointment requests |
| `gh-web` | Service | Single Next.js app, builds from `apps/web` |
...
`REVALIDATE_SECRET` is optional for v1. It is only needed if external systems outside the app need to trigger cache invalidation.

### 11.4 CI/CD

Railway's GitHub integration: every push to `main` triggers a build for the single app. Preview deployments on PRs.
...
Design the schema so data subject requests can be supported, but the dedicated admin tooling page (`/admin/data-requests`) is v1.1. For v1, requests can be handled manually by an admin with database access and a documented procedure:
...
- **Right to object / withdraw consent**: Form on the public website `/privacy/withdraw-consent`.
...
- **Cookies**: `httpOnly: true`, `secure: true` (production), `sameSite: 'lax'`, `path: '/'` on the main domain, scoped by middleware and route protection
...
- `apps/web/middleware.ts` checks JWT on every `/admin/*` route except `/admin/login`
...
2. `pnpm typecheck` (public and admin areas)
3. `pnpm lint`
4. `pnpm test:unit`
5. `pnpm test:integration` (with test DB)
6. `pnpm build` (public and admin areas)
...
```bash
pnpm dev                          # Single app (public + /admin)
pnpm --filter web dev             # Single app only (port 3000)
...
pnpm typecheck                    # App + packages
pnpm lint                         # ESLint everything
pnpm test                         # All tests
pnpm build                        # Production build, single app
```
...
`apps/web/app/admin/actions/services.ts`:
...
export async function updateService(input: unknown) {
  ...
  // 6. No external revalidate ping needed: public and admin live in the same app
...
`apps/web/app/api/admin/upload/presign/route.ts`:
...
### 17.5 Cache invalidation after admin writes

Because the public website and admin area are in the same Next.js app, admin server actions can invalidate public content directly with `revalidateTag()`.
...
export function revalidateServiceContent(serviceId: string, countrySlug: string) {
  revalidateTag(`service:${serviceId}`);
  revalidateTag(`country:${countrySlug}:services`);
  revalidateTag(`country:${countrySlug}`);
}
```

Use a public `/api/revalidate` endpoint only if a future external system needs to invalidate cache from outside the app.


### 17.6 Country dropdown — single source of truth

`apps/web/components/admin/CountryPicker.tsx`:
...
export function CountryPicker({ countries, current }: Props) {
  ...
  function selectCountry(slug: string) {
    ...
    const match = pathname.match(/^\/admin\/[a-z]{2,3}(?=\/|$)/);
    if (match) {
      const newPath = pathname.replace(/^\/admin\/[a-z]{2,3}/, `/admin/${slug}`);
      router.push(newPath);
      return;
    }
...
- ☐ Public website reflects admin changes within 30 seconds
- ☐ Brand identity is consistent across public and admin areas (verified by visual audit)
...
- ☐ Production env vars set, main domain active, SSL valid
...
- Patient register/login on the public website
...
|---|---|---|
| Stage 6 | One Next.js app with protected `/admin` routes | Business wants one website/codebase. Admin remains isolated by route protection, role checks, and middleware. |
